...
(function () {
  ...
  StringView = function (vInput, sEncoding, nOffset, nLength) {
    var FTAView,
      aWhole,
      aRaw,
      fPutOutptCode,
  ...
  conversionSwitch:
      ...
      case 0:
        ...
        for (nOutptIdx = 0; nOutptIdx < nOutptLen; nOutptIdx) {
      ...
      case 1:
        ...
        /* mapping... */
        nOutptLen = 0;
        for (nInptIdx = nStartIdx; nInptIdx < nEndIdx; nInptIdx++) {
          nOutptLen += fGetOutptChrSize(vSource[nInptIdx]);
        }
        aWhole = new FTAView(nOutptLen);
        /* transcription of the source... */
        nOutptIdx = 0;
        for (nInptIdx = nStartIdx; nOutptIdx < nOutptLen; nInptIdx++) {
      ...
      case 3:
        ...
        /* transcription... */
        nOutptIdx = 0;
        for (nInptIdx = nStartIdx; nOutptIdx < nOutptLen; nInptIdx += fGetInptChrSize(nChrCode)) {
          nChrCode = fGetInptChrCode(vSource, nInptIdx);
      ...
      case 5:
        ...
        /* mapping... */
        nOutptLen = 0;
        for (nMapIdx = 0; nMapIdx < nInptLen; nMapIdx++) {
  ...
  StringView.putUTF8CharCode = function (aTarget, nChar, nPutAt) {
    ...
    if (nChar < 0x80) {  /* 128 */
      ...
      /* two bytes */
      aTarget[nIdx++] = 0xc0 + (nChar >>> 6);
      aTarget[nIdx++] = 0x80 + (nChar & 0x3f);
    } else if (nChar < 0x10000) {
      /* three bytes */
      aTarget[nIdx++] = 0xe0 + (nChar >>> 12);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 6) & 0x3f);
      aTarget[nIdx++] = 0x80 + (nChar & 0x3f);
    } else if (nChar < 0x200000) {
      /* four bytes */
      aTarget[nIdx++] = 0xf0 + (nChar >>> 18);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 12) & 0x3f);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 6) & 0x3f);
      aTarget[nIdx++] = 0x80 + (nChar & 0x3f);
    } else if (nChar < 0x4000000) {
      /* five bytes */
      aTarget[nIdx++] = 0xf8 + (nChar >>> 24);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 18) & 0x3f);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 12) & 0x3f);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 6) & 0x3f);
      aTarget[nIdx++] = 0x80 + (nChar & 0x3f);
    } else {
      ...
      aTarget[nIdx++] = 0xfc + (nChar / 1073741824);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 24) & 0x3f);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 18) & 0x3f);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 12) & 0x3f);
      aTarget[nIdx++] = 0x80 + ((nChar >>> 6) & 0x3f);
      aTarget[nIdx++] = 0x80 + (nChar & 0x3f);
  ...
  StringView.loadUTF16CharCode = function (aChars, nIdx) {
    ...
    return nFrstChr > 0xD7BF && nIdx + 1 < aChars.length ?
        (nFrstChr - 0xD800 << 10) + aChars[nIdx + 1] + 0x2400
        : nFrstChr;
  };
  ...
  StringView.bytesToHex = function (buf) {
    ...
    if (buf.byteLength > 0) {
      str.push(("00" + bytes[0].toString(16)).substr(-2));
    }
    for (i = 1; i < buf.byteLength; i++) {
      if (i % 16 === 0) {
        str.push("    ");
        for (j = i - 16; j < i; j++) {
          str.push(String.fromCharCode(bytes[j]).replace(/\s/, " "));
        }
        str.push('\n');
      } else if (i % 8 === 0) {
        str.push("  ");
  ...
  StringView.prototype.toBase64 = function (bWholeBuffer) {
    return StringView.bytesToBase64(
      bWholeBuffer ?
          (this.bufferView.constructor === Uint8Array ? this.bufferView : new Uint8Array(this.buffer))
  ...
  StringView.prototype.forEachChar = function (fCallback, oThat, nChrOffset, nChrLen) {
    ...
    if (this.encoding === "UTF-8" || this.encoding === "UTF-16") {
      ...
      for (nChrIdx = 0; nRawIdx < nRawEnd; nChrIdx++) {
  ...
  StringView.prototype.valueOf = StringView.prototype.toString = function () {
    ...
    for (nIdx = 0; nIdx < nLen; nIdx += fGetIncr(nChr)) {
